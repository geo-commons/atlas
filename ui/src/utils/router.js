export const getSettingsFromPath = (defaultConfig) => {
    ...
    return {
        position: {
            zoom: match.groups.zoom ? parseFloat(match.groups.zoom) : defaultConfig.position.zoom,
            center: [
                match.groups.x ? parseFloat(match.groups.x) : defaultConfig.position.center.x,
