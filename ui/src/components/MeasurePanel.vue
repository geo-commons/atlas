<template>
  <div class="wrapper">
    <button class="iconbutton" :class="{ isActive: this.tool === 'MEASURE_AREA' }" @click="() => toggleTool('MEASURE_AREA')" aria-label="Oppervlakte meten">
...
export default {
  name: 'MeasurePanel',
  methods: {
    toggleTool(chosenTool) {
      if (chosenTool !== this.tool) {
        this.$emit('set-tool', chosenTool)
      } else {
        // toggle tool off when it is clicked again
        this.$emit('set-tool', '')
      }
    },
  },
