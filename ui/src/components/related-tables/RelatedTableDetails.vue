          ...
          <table>
            <tbody>
              ...
              <tr v-for="property in Object.keys(templateFields)" :key="property">
                ...
                <td>
                  <MarkdownTemplate :source="templateFields[property]" :data="relatedTableData" />
                </td>
              </tr>
            </tbody>
          </table>
...
const templateFields = computed<Record<string, string>>(() => {
  if (relatedTable.value?.template_fields) {
    return {
      ...relatedTable.value.template_fields,
    };
  }
...
const getRelatedTable = async (tableId: number) => {
  const response = await fetch(`/atlas/api/v1/tables/${tableId}/`);
...
const handleSelectedRelatedTableAttributes = async () => {
  ...
  const relatedData = await getRelatedTableData(relatedTable.value, selectedRelatedTableAttributes.item);
