<template>
  <div class="wrapper">
    <div class="header">
      <input type="text" ref="input" name="embedCode" class="input" :value="embedCode" readonly="readonly" />
      <button class="iconbutton" type="button" @click="copyHTML" :aria-label="buttonText">
...
export default {
  ...
  computed: {
    embedUrl() {
      return `${encodeURI(window.location.origin)}/atlas/embed/@${encodeURIComponent(this.position.center[0])},${encodeURIComponent(this.position.center[1])},${encodeURIComponent(this.position.zoom)}z/layers=${this.visibleLayers.map((l) => encodeURIComponent(l.id)).join(',')}`
    },
    embedCode() {
